...
typedef struct play_control_t
    ...
    musicdb_song_t tracks[ 256 ];
    int tracks_shuffle[ 256 ];
    int tracks_count;
    int track_index;
...
bool play_control_update( play_control_t* control )
    {
    if( control->album_id == MUSICDB_INVALID_ID ) return false;
    if( play_thread_song_finished( control->play_thread ) )
        ...
        play_thread_queue_song( control->play_thread, control->repeat || next_index != 0 ? filename : "" );
        return true;
        }
...
void play_control_track( play_control_t* control, int track_index )
    {
    if( control->album_id == MUSICDB_INVALID_ID ) return;
...
float play_control_length( play_control_t* control )
    ...
    if( control->album_id == MUSICDB_INVALID_ID ) return 0;
    int track_index = control->track_index;
